...
std::vector<uint8_t> Crypto::DeriveKey(const std::string& password, const std::vector<uint8_t>& salt) {
...
std::vector<uint8_t> Crypto::HashKey(const std::vector<uint8_t>& key) {
...
bool Crypto::Encrypt(const std::string& plaintext, const std::vector<uint8_t>& key, 
    ...
    if (EVP_EncryptInit_ex(ctx, EVP_aes_256_gcm(), nullptr, nullptr, nullptr) == 1 &&
        EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_GCM_SET_IVLEN, static_cast<int>(out_iv.size()), nullptr) == 1 &&
        EVP_EncryptInit_ex(ctx, nullptr, nullptr, key.data(), out_iv.data()) == 1 &&
        EVP_EncryptUpdate(ctx, out_ciphertext.data(), &len, 
                          reinterpret_cast<const uint8_t*>(plaintext.data()), static_cast<int>(plaintext.length())) == 1) {
        
        ciphertext_len = len;
        
        if (EVP_EncryptFinal_ex(ctx, out_ciphertext.data() + len, &len) == 1) {
            ciphertext_len += len;
            out_ciphertext.resize(ciphertext_len);
            
            if (EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_GCM_GET_TAG, 16, out_auth_tag.data()) == 1) {
                success = true;
            }
        }
    }
...
bool Crypto::Decrypt(const std::vector<uint8_t>& ciphertext, const std::vector<uint8_t>& iv, 
    ...
    if (EVP_DecryptInit_ex(ctx, EVP_aes_256_gcm(), nullptr, nullptr, nullptr) == 1 &&
        ...
        EVP_DecryptUpdate(ctx, plaintext_buffer.data(), &len, ciphertext.data(), static_cast<int>(ciphertext.size())) == 1) {
        
        plaintext_len = len;
        
        // Cast away constness safely as OpenSSL API requires void* for this specific control operation
        EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_GCM_SET_TAG, static_cast<int>(auth_tag.size()), 
                            const_cast<void*>(reinterpret_cast<const void*>(auth_tag.data())));

        if (EVP_DecryptFinal_ex(ctx, plaintext_buffer.data() + len, &len) > 0) {
            plaintext_len += len;
...
std::vector<uint8_t> Crypto::GenerateRandomBytes(size_t size) {
...
std::string Crypto::GeneratePassword(size_t length) {
